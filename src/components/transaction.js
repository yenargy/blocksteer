            ...
            <ToastHub timeout={500}>
              <Toast>
                {toast => (
                  <TransactionItem 
                    hoverEnabled
                    onClick={() => copyTextToClipboard(props.to ? props.to : props.contractAddress) ? toast("To address copied to clipboard") : toast("Failed to copy the block hash. Please try again.")}>
                    {props.input === '0x' ?
                      <IconUser size="small" css={`opacity: 0.6;`} />
                      :
